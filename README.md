...
Before AMO submission, replace `network-speed-monitor@example.local` in `manifest.json` with an owned, stable add-on ID. Run `npm run package`, then submit `web-ext-artifacts/network_speed_monitor-1.0.0.zip`. AMO reviewers may also request the corresponding source repository.
