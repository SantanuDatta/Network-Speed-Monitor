...
function renderNavigationRow(icon: IconName, label: string, hash: string): string {
  return `<a class="navigation-row" href="dashboard.html#${hash}" target="_blank" rel="noopener">${heroIcon(icon)}<span>${label}</span>${heroIcon("chevron-right", "navigation-chevron")}</a>`;
}
  ...
  setPopupMarkup(
    ...
    <section class="popup-shell shadcn-popup">
      <header class="popup-header">
        ...
        <a class="icon-button settings-button" href="dashboard.html#settings" target="_blank" rel="noopener" title="Open settings" aria-label="Open settings"><img class="settings-cog" src="assets/icons/cog-wheel.svg" width="18" height="18" alt="" /></a>
      </header>
      ...
      <section class="ui-card summary-card ${settings.precision === 7 ? "precision-expert" : ""}" aria-label="Today's connection summary">
        <div class="summary-heading"><span>Today</span><span class="live-dot">Live</span></div>
...
function bindPopupEvents(snapshot: ExtensionSnapshot): void {
  document.querySelectorAll<HTMLAnchorElement>('a[target="_blank"]').forEach((link) => {
    link.addEventListener("click", () => window.setTimeout(() => window.close(), 0));
  });
